...
class DeptChoose(forms.Form):
    ...
    answer = SelectAnswer(query_text='SELECT name FROM mm.deps d').selecting()
    print(answer)
    if type(answer) is str:
        dept_name = 'Ошибка выборки подразделений.'
    else:
        print()
        ...
        @staticmethod
        def converting(x):
            """ Defined method that create tuple of sequence of tuples that
                send to form the choices list on the page.
            """
            result = []
            for i in x:
                result.append((i, i))
